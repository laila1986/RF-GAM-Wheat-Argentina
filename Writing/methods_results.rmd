...
```{r yield-eonr-dist, fig.cap = "Distribution of EONR and YEONR by field", fig.dim = c(7, 9), cache = F}
g_eonr_yield_dist
```

```{r rf-shap, fig.cap = "Shapley values of top five contributing factors for RF-estimated EONR by field", fig.dim = c(7, 9), cache = F}
rf_g_shap
```

```{r gam-shap, fig.cap = "Shapley values of top five contributing factors for GAM-estimated EONR by field", fig.dim = c(7, 9), cache = F}
gam_g_shap
```
